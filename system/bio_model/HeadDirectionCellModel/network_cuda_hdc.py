...
from system.bio_model.HeadDirectionCellModel.neuron import tau, phi, r_m, beta_T, h_0
# from params import n_x,n_y,tau_gc
# pycuda
...
class CudaInterface:
    def __init__(self, topology):
        ...
        maxThreadPerBlock = cuda.Device.get_attribute(
            device, cuda.device_attribute.MAX_THREADS_PER_BLOCK)
        maxThreadX = cuda.Device.get_attribute(
            device, cuda.device_attribute.MAX_BLOCK_DIM_X)
        self.threadLength = min([maxThreadPerBlock, maxThreadX])
        self.gridLength = cuda.Device.get_attribute(
            device, cuda.device_attribute.MAX_GRID_DIM_X)
        # compile cuda code
        self.registerModule()
        # save cuda functions
        ...
        self.update_rates_prep = self.update_rates.prepare(
            ["P", "P", "P", "P", "P" , "f", "i"])
        self.sum_rates_prep = self.sum_rates.prepare(
            ["P", "P", "P", "i", "i", "i"])
    ...
    def registerModule(self):
        ...
        cudaModStr = """
            ...
            __global__ void update_rates(float *rates ,float *input_before,float *rates_input, float *currents, float *currentsExternal, float dt, int offset) {
                // same function as in numpy implementation
                float totalInput = currents[offset + threadIdx.x] + currentsExternal[offset + threadIdx.x];
                rates_input[offset + threadIdx.x]=rates[offset + threadIdx.x]+totalInput;
                rates[offset + threadIdx.x]= rates_input[offset + threadIdx.x]+tau*(rates_input[offset + threadIdx.x]/sqrt(input_before[offset + threadIdx.x])-rates_input[offset + threadIdx.x]);
                if (rates[offset + threadIdx.x]<0){
                    rates[offset + threadIdx.x]=0;
                }
            }
    ...
    def loadNetwork(self, topology):
        ...
        # firing rates
        self.rates_cpu=np.random.rand(self.topology.totalNeurons)/(self.topology.totalNeurons/5)**0.5
        self.rates_cpu=self.rates_cpu.astype(np.float32)
        self.rates_gpu = cuda.mem_alloc(self.rates_cpu.nbytes)
        cuda.memcpy_htod(self.rates_gpu, self.rates_cpu)

        # external input currents
        self.currents_cpu = np.zeros(
            self.topology.totalNeurons, dtype=np.float32)
        ...
        #initial input_before
        self.input_before_cpu=np.random.rand(self.topology.totalNeurons)/(self.topology.totalNeurons/5)**0.5
        self.input_before_cpu=self.input_before_cpu.astype(np.float32)
        self.name_list=['hdc_attractor','hdc_shift_left','hdc_shift_right','Conj','ecd_ring','acd_ring','Conj2','hdc_ring_2']
        for name in self.name_list:
            (layerStart,layerLength) = self.topology.getLayer(name)
            self.input_before_cpu[layerStart:layerStart+layerLength]=np.ones(layerLength)*np.sum(self.input_before_cpu[layerStart:layerStart+layerLength])
        self.input_before_gpu=cuda.mem_alloc(self.input_before_cpu.nbytes)
        cuda.memcpy_htod(self.input_before_gpu, self.input_before_cpu)
        # initial rates_input
        self.rates_input_cpu=np.zeros(self.topology.totalNeurons, dtype=np.float32)
        self.rates_input_gpu=cuda.mem_alloc(self.rates_input_cpu.nbytes)
        cuda.memcpy_htod(self.rates_input_gpu, self.rates_input_cpu)
        ...
        for entry in topology.vectorSynapses:
            ...
            while remaining > self.threadLength:
                vectorSynapses.append(((startT + offset, self.threadLength),
                                       (startF, lengthF), matrix[offset: offset + self.threadLength]))
                remaining -= self.threadLength
                offset += self.threadLength
            if remaining != 0:
                vectorSynapses.append(
                    ((startT + offset, remaining), (startF, lengthF), matrix[offset: offset + remaining]))
        ...
        for entry in vectorSynapses:
            ...
            self.vectorSynapses.append(
                ((startT, lengthT), (startF, lengthF), matrix_gpu))
    ...
    def step(self, dt, debug=False, numsteps=1):
        ...
        if self.currents_changed:
            cuda.memcpy_htod_async(
                self.currents_gpu, self.currents_cpu, stream=self.stream)
            self.currents_changed = False
        for _ in range(numsteps):
            ...
            for entry in self.vectorSynapses:
                ...
                self.sum_rates.prepared_async_call((1, 1), (lengthT, 1, 1), self.stream, matrix, self.rates_gpu, self.inputs_gpu, np.int32(
                    lengthF), np.int32(startT), np.int32(startF))
            ################# update rates #########################
            # run on gpu
            ...
            while remaining > self.threadLength:
                ...
                self.update_rates.prepared_async_call((1, 1), (self.threadLength, 1, 1), self.stream,
                                                      self.rates_gpu,self.input_before_gpu,self.rates_input_gpu, self.inputs_gpu, self.currents_gpu, np.float32(dt), np.int32(start))
                remaining -= self.threadLength
            ...
            self.update_rates.prepared_async_call(
                (1, 1), (remaining, 1, 1), self.stream, self.rates_gpu,self.input_before_gpu,self.rates_input_gpu, self.inputs_gpu, self.currents_gpu, np.float32(dt), np.int32(start))
        # transfer result to CPU
            cuda.memcpy_dtoh_async(
                self.rates_cpu, self.rates_gpu, stream=self.stream)
            cuda.memcpy_dtoh_async(
                self.rates_input_cpu, self.rates_input_gpu, stream=self.stream)
            for name in self.name_list:
                (layerStart,layerLength) = self.topology.getLayer(name)
                self.input_before_cpu[layerStart:layerStart+layerLength]=np.ones(layerLength)*np.sum(self.rates_input_cpu[layerStart:layerStart+layerLength])
            cuda.memcpy_htod(self.input_before_gpu, self.input_before_cpu)
        if debug:
            print("step time: {:.6f}ms, synapse count: {}".format(float(time.time(
            ) - t_start) * 1000.0, sum([m.shape[0] * m.shape[1] for (_, _, m) in self.topology.vectorSynapses])))
        ...
        for i in range(startT, startT + lengthT):
            if i in self.synapses.keys():
                ...
                if len(syn.keys()) == 0:
                    del self.synapses[i]
        self.vectorSynapses.append(
            ((startT, lengthT), (startF, lengthF), matrix))
            ...
            def __init__(self, layers, layer):
                ...
                if layerCnt == 0:
                    ...
                    for k in layers.keys():
                        print("{}: {} neurons, ids {}-{}".format(k,
                                                                 layers[k][1], layers[k][0], layers[k][0] + layers[k][1] - 1))
