...
for index in range(p.nrTransformationLayers):
    ...
    heading2TransformationWts[:, :, index] = np.outer(np.ones(num_ray*16), HDCrate)

#rescaling
divtmp = np.zeros((num_ray*16, num_ray*16))
for index in range(20):
    ego2TransformationWts[:, :, index] = ego2TransformationWts[:, :, index] / np.amax(ego2TransformationWts[:, :, index])
    divtmp = np.outer(np.sum(ego2TransformationWts[:, :, index], 1), np.ones(num_ray*16))
...
np.save("weights/ego2TransformationWts", ego2TransformationWts)
np.save("weights/heading2TransformationWts", heading2TransformationWts)
np.save("weights/transformation2BVCWts", transformation2BVCWts)
