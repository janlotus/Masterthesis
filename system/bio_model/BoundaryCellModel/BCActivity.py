...
def boundaryCellActivityTraining(boundary_location):
    ...
    # polar_angles = np.linspace(0, 2 * math.pi, num_ray,endpoint=False)
    polar_angles = np.linspace(0, 2 * math.pi, num_ray,endpoint=True)
    ...
    activity_vector = np.zeros((num_ray*16, 1))
...
def boundaryCellActivitySimulation(thetaBndryPts, rBndryPts):
    ...
    '''
    polar_distances = subroutines.radialScaling()
    # polar_angles = np.linspace(0, 2 * math.pi, num=num_ray,endpoint=False) # hier should Endpoint=False# - parametersBC.polarAngularResolution
    # polar_angles = np.linspace(0, 2 * math.pi, num=num_ray,endpoint=True) # hier should Endpoint=False# - parametersBC.polarAngularResolution
    resolulation=math.pi/(num_ray*2)
    polar_angles = np.linspace(-resolulation, 2 * math.pi-resolulation, num=num_ray,endpoint=False) # - parametersBC.polarAngularResolution[0,..6.28]
    ...
    grid_angle = np.reshape(np.transpose(p_ang), (np.prod(np.size(p_ang)), 1), order="F")  # corresponding angles
    transform_matrix = np.where(grid_angle > math.pi, 1, 0)
    grid_angle = grid_angle - 2 * math.pi * transform_matrix # 0-2pi to 0... pi..-pi..0
    activity_vector = np.zeros((num_ray*16, 1))
