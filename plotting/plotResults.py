...
def cognitive_map_plot(pc_network, cognitive_map, xy_coordinates=None, pc_active_array=None, environment=None):
    ...
    for i, pc in enumerate(pc_network.place_cells):
        ...
        circle = plt.Circle((goal_pos[0], goal_pos[1]), 0.12,
                            ...
                            ec=TUM_colors['TUMGray'], linewidth=0)
        ax.add_artist(circle)
        cue_pos = environment.cuePos  # cue position from environment
        circle_cue = plt.Circle((cue_pos[0], cue_pos[1]), 0.12,
...
def plot_linear_lookahead(f_gc, f_t, f_mon, frame, gc_network, xy_coordinates=None, reward_array=None, goal_found=None,env=None):
...
def export_linear_lookahead_video(gc_network, filename, xy_coordinates=None, reward_array=None, goal_found=None,env=None):
