...
# --------------- Plot cognitive map ---------------
def cognitive_map_plot(env,pc_network, cognitive_map, vectors_array=None):

    plt.figure()

    ax = plt.gca()
    add_cognitive_map(env,ax, pc_network, cognitive_map)
    ...
    plt.ylim(-0.5, 11.5)
    # path = os.path.abspath(os.path.join(os.getcwd(), '..'))
    # plt.savefig(path+"/experiments/" + "cognitive_map_exploration_phase", format="pdf")
    directory = "experiments/"
    if not os.path.exists(directory):
        os.makedirs(directory)
    plt.savefig("experiments/" + "cognitive_map_exploration_phase.pdf", format="pdf")
    plt.show()


def add_cognitive_map(env,ax, pc_network, cognitive_map):
    for i, pc in enumerate(pc_network.place_cells):
        ...
        # goal position
        # goal position (1.5,10)
        goal_pos=env.goal_location
        # circle = plt.Circle((1.5, 10), 0.12,
        #                     fc=TUM_colors['TUMBlue'],
        #                     ec=TUM_colors['TUMGray'], linewidth=0)
        circle = plt.Circle((goal_pos[0], goal_pos[1]), 0.12,
                            fc=TUM_colors['TUMBlue'],
                            ec=TUM_colors['TUMGray'], linewidth=0)
        ax.add_artist(circle)
        cue_pos=env.cuePos # cue position from env
        circle_cue = plt.Circle((cue_pos[0], cue_pos[1]), 0.2,
                            fc=TUM_colors['TUMAccentGreen'],
                            ec=TUM_colors['TUMGray'], linewidth=0)
        ax.add_artist(circle_cue)


def add_environment(ax,env):
    # doors option:"plane","plane_doors_open35","plane_doors_open15","plane_doors_open1","plane_doors_allclose"
...
def plot_sub_goal_localization(env, cognitive_map, pc_network, goal_vector, filename, chosen_idx=0, goal_spiking=None):
    ...
    plt.xlim(-0.5, 11.5)
    plt.ylim(-0.5, 11.5)
    plt.savefig("experiments/" + "goal_lookahead" + filename+".pdf", format="pdf")
    # plt.show()
    # plt.close()
...
def plot_trajectory(xy_coordinates, door,env):
    ...
    directory = "experiments/maze_navigation/" + door + "/plots/"
    if not os.path.exists(directory):
        os.makedirs(directory)
    plt.savefig("experiments/maze_navigation/" + door + "/plots/" + "trajectory_initial", format="pdf")
    # plt.savefig("experiments7" + "trajectory_initial", format="pdf")
    plt.show()
    plt.close()
